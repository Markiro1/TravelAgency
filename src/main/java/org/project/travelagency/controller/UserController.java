...
public class UserController {
    ...
    public String read(@PathVariable("user_id") long userId, Model model) {
        ...
        model.addAttribute("user", user); // map to dto
        return "user-info";
    }
    ...
    public String update(@PathVariable("user_id") long userId, Model model) {
        User user = userService.readById(userId);
        model.addAttribute("user", UserCreateMapper.mapToDto(user)); // !!! orders need to update
        return "update-user";
    }

    @PostMapping("/{user_id}/update")
    public String update(@PathVariable("user_id") long userId, Model model,
                         @Validated @ModelAttribute("user") UserCreateDto userDto,
        ...
        userService.update(userDto); // !!! orders need to update
